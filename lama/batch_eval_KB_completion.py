...
import time, sys
from lama_utils import parse_template, load_vocab, load_file
        ...
        if "obj_label" in sample and "sub_label" in sample:
            ...
            #   samples_exluded+=1
                ...
                if num_no > num_yes:
                    ...
                    new_samples.append(sample)
                    # print(sample["obj_label"],obj_label_ids)
            else:
                new_samples.append(sample)
                # print(sample["obj_label"],obj_label_ids)
        else:
...
def run_evaluation(args, shuffle_data=True, model=None):
    ...
    for i in tqdm(range(len(samples_batches))):
        ...
        # print('mask:',len(masked_indices_list))
        # print('mask:',masked_indices_list)
        # print('token:',token_ids_list)
        ...
        # print('prob:',filtered_log_probs_list)
        # print('prob:',filtered_log_probs_list.shape)
        ...
        for idx, result in enumerate(res):
            ...
            # with open('./output_roberta.txt', 'a+') as f:
            #     f.write('\n')
            #     # print()
            #     f.write("idx: {}".format(idx)+'\n')
            #     f.write("masked_entity: {}".format(label_index_list[idx])+'\n')
            #     for yi in range(10):
            #         f.write("\t{} {}".format(yi,result_masked_topk['topk'][yi])+'\n')
            #     f.write("masked_indices_list: {}".format(masked_indices_list[idx])+'\n')
            #     f.write("sample_MRR: {}".format(sample_MRR)+'\n')
            #     f.write("sample_P10: {}".format(sample_P)+'\n')
            #     f.write("sample_P: {}".format(result_masked_topk["P_AT_1"])+'\n')
            #     f.write("sample: {}".format(sample)+'\n')
            #     f.write('\n')
            # with open('./input.json', 'a+') as f2:
            #     # json.dump(sample, f, indent=4)
            #     f2.write("{}".format(sample)+',\n')
            # if element["sample_Precision1"]>0:
            #     with open('./true.txt', 'a+') as f3:
            #         # f3.write('\n')
            #     # print()
            #         f3.write("idx: {}".format(idx)+'\n')
            #         f3.write("masked_entity: {}".format(label_index_list[idx])+'\n')
            #         for yi in range(10):
            #             f3.write("\t{} {}".format(yi,result_masked_topk['topk'][yi])+'\n')
            #         f3.write("masked_indices_list: {}".format(masked_indices_list[idx])+'\n')
            #         f3.write("sample_MRR: {}".format(sample_MRR)+'\n')
            #         f3.write("sample_P10: {}".format(sample_P)+'\n')
            #         f3.write("sample_P: {}".format(result_masked_topk["P_AT_1"])+'\n')
            #         f3.write("sample: {}".format(sample)+'\n')
            #         f3.write('\n')
            MRR += sample_MRR
            Precision += sample_P
