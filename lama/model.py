...
from transformers import RobertaForMaskedLM, RobertaTokenizer, BertForMaskedLM, BertTokenizer, RobertaModel, RobertaConfig
...
class Roberta(object):
    ...
    def __init__(self, args):
        ...
        if args.model_path is not None:
            ...
            matched_parameters = {}
            exclude = 'token_type_embeddings'
            include = 'text'
            for name in model_dict:
                for k, v in pretrained_dict.items():
                    # print("all:",name, k)
                    if name in k and include in k and exclude not in name:
                        matched_parameters[name]=v
            print('parameters below are randomly initializecd:')
            for name, param in self.model.named_parameters():
                if name not in matched_parameters:
                    print(name)
    ...
    def init_indices_for_filter_logprobs(self, vocab_subset):
        index_list = []
        new_vocab_subset = []
        # f = open('/home/yuanxiaowei/lama/filter_words.txt','w')
    ...
    def get_batch_generation(self, samples_list, try_cuda=True):
        ...
        for sample in samples_list:
            ...
            masked_inputs_list = [masked_inputs_list[0].replace('[MASK]','<mask>')]
